...
function Dashboard() {
  ...
  const getCurrentTabFromPath = () => {
    ...
    Object.values(TAB_URLS).forEach((tabUrl) => {
  ...
  const [tab, setTab] = useState(getCurrentTabFromPath())
  ...
  return (
    ...
    >
      <Box
        as={Link}
        to="https://www.youtube.com/@hwinkdev.official"
        target="blank"
      >
        <Box
          component="img"
      ...
      <TabContext value={tab}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <TabList
            ...
            aria-label="Hwinkdev RBAC Permissions Tabs"
          >
        ...
        {hasPermission(permissions.VIEW_DASHBOARD) && (
          <TabPanel value={TAB_URLS.DASHBOARD}>
            <Alert severity="success" sx={{ width: 'max-content' }}>
              Đây là nội dung của tab Dashboard, tất cả user đã đăng nhập đều có
              thể truy cập vào tab này.
            </Alert>
          </TabPanel>
        )}
